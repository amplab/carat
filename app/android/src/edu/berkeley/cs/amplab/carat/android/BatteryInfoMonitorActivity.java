...
public class BatteryInfoMonitorActivity extends Activity {
	...
	/*
	 * Why do we do this? Why unregister? Sampling should be active on the
	 * background and always record samples at intervals of <= 1 min.
	 */
	...
	/* GUI broadcastreceiver. Should be removed in favor of starting GUI updates after Sampling
	 * by calling a callback method. */
				...
				BatteryInfoMonitorActivity.this.runOnUiThread(new Runnable() {
					...
					public void run() {
						String tmp1 = "CPU Use Details:  \n"
								...
								+ String.valueOf(totalCpuTime) + " Sec\n"
								+ "Total cpu Idle time:"
						...
						// TotalRunProcNum.setText("Current total number of running processes:"
						// + runningProcess.size());
						...
						for (Iterator<RunningAppProcessInfo> iterator = runningProcess
								.iterator(); iterator.hasNext();) {

							RunningAppProcessInfo RunningProcList = iterator
									.next();
							HashMap<String, String> map = new HashMap<String, String>();
							map.put("processPID", "The process PID is:"
									+ String.valueOf(RunningProcList.pid)
									+ "\n");
							map.put("userid",
									"The user ID is:"
											+ String.valueOf(RunningProcList.uid)
											+ "\n");
							map.put("processName", "The process name is:"
									+ RunningProcList.processName + "\n");
							map.put("importance",
									"The importance level is:"
											+ String.valueOf(RunningProcList.importance)
											+ "\n");
							map.put("lru",
									"The lru is:"
											+ String.valueOf(RunningProcList.lru)
											+ "\n");
							...
							map.put("memory", "The occupied memory size is:"
	...
	private BroadcastReceiver batteryBroadcastRecv = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (ac.equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				sbattery.append("The battery level is:").append(Batterylevel)
						.append("%");
